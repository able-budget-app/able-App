...
- 2026-05-04 | **Let user choose snowball vs avalanche debt payoff method** | Today the Debt tab implicitly orders by interest rate (avalanche — pay highest APR first). Add a setting toggle: Avalanche (math-optimal, highest APR first) or Snowball (Dave Ramsey, smallest balance first for psychological wins). Ramsey followers will explicitly want snowball — losing them as customers because we don't offer it is unforced. Default avalanche, surface the choice in Debt settings. Reorder "Pay this first / Pay this next" + payoff timeline based on the choice. | Audience overlap with Ramsey/PF readers; explicit choice respects user autonomy
