...
- ~~**P1-2026-05-08 #1 — Misleading "Refresh plan" banner on home.**~~ ✅ Shipped 2026-05-11. Tightened the gating in `renderPlanRefreshBanner` (`app.html:10678`) so the banner only fires when (a) the user has bills (plan drafted), (b) ≥2 plaid_items exist, and (c) a non-oldest item is `recurring_status='fresh'` AND was created >1h after the oldest (rules out back-to-back onboarding adds + routine recurring-webhook refreshes on the original bank). Rewrote copy to "[Bank name] connected — Check Bills + Income for new entries to fold into your plan." CTA renamed to `reviewNewBankFromBanner(itemId)` — flips just that item to `'applied'` and routes to Bills (no LLM, no analyzer call, one-shot per bank). Dismiss path symmetric. Pairs with P1-2026-05-06 #2 below — the banner CTA will swap from `goTo('bills')` → opens the Option-C combined review modal when that ships.
...
- ~~**P1-2026-05-07 #2 — Onboarding tax calibration by state.**~~ ✅ Already shipped — confirmed wired end-to-end on 2026-05-08 evening review. `OB_QUESTIONS` line 3877 has the `tax_state` question; `S.profile.state` persists at lines 4576/6335; `profile_hint.state` passed to analyzer at line 5151; analyzer prompt Rule 4 (lines 963-968 of `plaid-analyze/index.ts`) maps state codes to tax bands (FL/TX/NV/SD/TN/WA/WY/AK/NH add 0; CO/IN/MI/NC/ND/PA/UT add 2-3; AR/GA/IA/KS/LA/ME etc. add 3-5; CA/NY/NJ/OR/HI add 5-7). Validated 2026-05-08 test signup: Idaho user got 25% suggested (22% baseline + 3 from Medium band) — exact match for Rule 4 expected output.
...
- ~~**P1-2026-05-06 #2 — Add-bank-from-settings only syncs, doesn't classify or analyze.**~~ ✅ Shipped 2026-05-11 (in addition to the PR #19 partial from 2026-05-06). Option-C delta UI built as `openNewBankReviewModal` / `applyNewBankReview` (`app.html:10753+`) + `#modal-new-bank-review` HTML scaffold (`app.html:2624`). Reads `plaid_recurring_streams` (outflow + active + filtered to the new item) and `plaid_credit_debts` (same filter) in two parallel SELECTs on open; renders a checkbox list of candidate bills + cards with pre-selected defaults; appends accepted rows to `S.bills` / `S.debts` with `plaid_item_id` + `source_plan_id='delta:<itemId>'` annotation. **Deterministic only — no `plaid-analyze` call, no LLM.** Dedupe by existing case-insensitive name match. Cost-bomb prevention: banner is gated on `recurring_status='fresh'` on a non-oldest item created >1h after the oldest; modal flips that item to `'applied'` on apply (one-shot per bank). Banner CTA in `renderPlanRefreshBanner` (`app.html:10728`) calls `openNewBankReviewModal(itemId, bankName)` directly. Deferred: income sources (require analyzer for work-derived naming — Rule 7) and the LLM rename of generic rail names. Revisit if real users complain about "Stripe payouts" / "ADP Payroll" generic naming after second-bank adds.
