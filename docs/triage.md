...
- **P0-2026-05-08 #1 — Mark-as-bill from Activity / Leaks doesn't propagate to Bills page.** Discovered in fresh-signup E2E test 2026-05-08 (user `pauljohnsontest11@paul7.com`, item `eed3eacd`). Paul marked Villa Sport as a bill via Activity (or Leaks tagging — verify which surface) and it didn't show up on Bills. Silent failure — the UI accepts the action but bills page state isn't updated. High user-trust impact: if users can't trust that "mark as bill" actually adds to bills, the leak-tagging UX is undermined. **Investigate:** what handler runs on the mark-as-bill action? Does it write to `user_data.bills`? Does Bills page read from a stale source vs. the new bill location? Possibly two separate writes (one for activity tagging, one for bills membership) and only the first happens. Likely live in `app.html` activity-detail handler + bills tab loader.

- **P0-2026-05-08 #2 — Analyzer runs before plaid_recurring_streams populates.** Race condition: analyzer fires at T+~50s after sync (when classify is ~6%, recurring streams = 0). Plaid's `RECURRING_TRANSACTIONS_UPDATE` webhook fires 2-10 min later, which then triggers `plaid-recurring-refresh`. So the first plan is built from raw transactions instead of Plaid's pattern-detected streams, producing systematically worse income / bill detection (10× off on Airbnb income in 2026-05-08 test — $355/mo plan vs $888/wk = ~$3,847/mo stream; 5+ recurring bills missed: Northwestern Mutual, Smartcredit, Apple, Google One, Villa Sport ≈ $120/mo). **Fix options:** (a) wait for `historical_sync_complete=true` AND `plaid_recurring_streams` count > 0 before kicking off analyzer (adds 2-10 min to onboarding which hurts time-to-value); (b) accept first plan from raw txns then auto-re-run analyzer once `RECURRING_TRANSACTIONS_UPDATE` webhook lands and silently refresh the plan (no UX change, but plan may shift after user has seen it); (c) hybrid — show first plan immediately but flash a "refining your plan with recurring detection..." indicator that resolves when v2 lands. Pick after thinking through UX of plan changing under user. Pairs naturally with B10 (in-app regenerate-plan failsafe).
