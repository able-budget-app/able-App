...
- **P1-2026-05-06 #2 — Add-bank-from-settings only syncs, doesn't classify or analyze.** ✅ Partially shipped 2026-05-06 via PR #19 (add_product flow + Enable APR tracking button so existing items can gain liabilities without full re-link). The credit-card detection portion now runs through the same `plaid-detect-credit-debts` function as the onboarding path. **Still open: Option-C delta UI.** Scope locked 2026-05-11: one combined "Here's what we found in [Bank Name]" modal per newly-added bank, surfacing bills + debts + income sources in a card-stack (same UX as onboarding plan review, scoped to `plaid_item_id == newBank.id`). **Deterministic detection only — no `plaid-analyze` call, no LLM rename in v1.** Everything already detected upstream by `plaid-classify-pending` + `plaid-detect-recurring` + `plaid-detect-credit-debts`; the modal is pure UI on existing data. Entry point: the new-bank banner shipped 2026-05-11 (CTA swaps from `goTo('bills')` → opens the modal). Cost-bomb prevention: gated on `recurring_status='fresh'`, flips to `'applied'` on modal completion (one-shot per bank). LLM rename of work-derived income source names (Rule 7) deferred — revisit if generic rail names like "Stripe payouts" actually bother real users.
