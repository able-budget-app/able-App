...
- **P1-2026-05-07 #1 — In-app monthly→annual upgrade (no Stripe Checkout redirect).** Today's "Switch to annual" path opens Stripe Checkout again, which Persona 7 flagged on Day 30: *"the Stripe checkout opening again on day 30 made my heart skip. I already paid you."* Replace with a Settings button that calls a new Edge Function (`switch-plan` or extend `create-checkout`) using `stripe.subscriptions.update(subId, { items: [{ id, price: PRICE_ANNUAL }], proration_behavior: 'always_invoice' })` against the saved payment method — charges the prorated difference immediately, no redirect. Likely lifts monthly→annual upgrade rate (synthesis-round2 follow-up #7 calls this out explicitly). Effort: S-M. Touches: new Edge Function, `app.html` Settings → Subscription card, copy ("Switch to annual — we'll charge the prorated difference today"). | Source: focus-group SYNTHESIS-round2 #7 + persona-7-lena-30day.md:176.
